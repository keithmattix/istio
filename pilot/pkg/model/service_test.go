...
func TestGetDNSConnectStrategy(t *testing.T) {
	tests := []struct {
		name        string
		annotations map[string]string
		want        DNSConnectStrategy
	}{
		{
			name:        "new annotation key enabled",
			annotations: map[string]string{"istio.io/connect-strategy": "RACE_FIRST_TCP_CONNECT"},
			want:        DNSConnectStrategyRaceFirstTCPConnect,
		},
		{
			name:        "new annotation key case insensitive value",
			annotations: map[string]string{"istio.io/connect-strategy": "race_first_tcp_connect"},
			want:        DNSConnectStrategyRaceFirstTCPConnect,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := GetDNSConnectStrategy(tt.annotations)
			if got != tt.want {
				t.Fatalf("GetDNSConnectStrategy() = %v, want %v", got, tt.want)
			}
		})
	}
}
